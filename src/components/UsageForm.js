...
function UsageForm() {
  ...
  const { setFormData, formData, setValidation, errors } = useContext(FormContext);
  ...
  return (
    <>
      <span className={`${classes.container} ${classes.headings}`}>
        <h1>How are you planning to use JDFinance?</h1>
        <p>We'll streamline your setup experience accordingly.</p>
      </span>

      { errors.usage ? <span style={colorStyle}>Please select an option</span> : null }
